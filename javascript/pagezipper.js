...
(function (E, B) {
  ...
  c.fn.extend({
    data: function (a, b) {
      ...
      k[1] = k[1] ? '.' + k[1] : '';
      if (b === B) {
        d = this.triggerHandler('getData' + k[1] + '!', [
          k[0]
        ]);
        if (d === B && this.length) {
          d = c.data(this[0], a);
  ...
  c.event = {
    ...
    trigger: function (a, b, d, e) {
      ...
      a.currentTarget = d;
      (e = d.nodeType ? c.data(d, 'handle')  : (c.data(d, '__events__') || {
      }).handle) && e.apply(d, b);
  ...
  (function () {
    ...
    var k = function (g, i, n, m) {
      ...
      if (D.length > 1 && x.exec(g)) if (D.length === 2 && o.relative[D[0]]) u = L(D[0] + D[1], i);
       else for (u = o.relative[D[0]] ? [
        i
      ] : k(D.shift(), i); D.length; ) {
        g = D.shift();
    ...
    k.filter = function (g, i, n, m) {
      for (var p, q, u = g, y = [
      ], F = i, M = i && i[0] && k.isXML(i[0]); g && i.length; ) {
    ...
    var o = k.selectors = {
      ...
      find: {
        ID: function (g, i, n) {
          if (typeof i.getElementById !== 'undefined' && !n) return (g = i.getElementById(g[1])) && g.parentNode ? [
            g
          ] : [
          ]
        },
        NAME: function (g, i) {
          if (typeof i.getElementsByName !== 'undefined') {
            for (var n = [
            ], m = i.getElementsByName(g[1]), p = 0, q = m.length; p < q; p++) m[p].getAttribute('name') === g[1] && n.push(m[p]);
    ...
    (function () {
      ...
      if (t.getElementById(i)) {
        o.find.ID = function (m, p, q) {
          if (typeof p.getElementById !== 'undefined' && !q) return (p = p.getElementById(m[1])) ? p.id === m[1] || typeof p.getAttributeNode !== 'undefined' && p.getAttributeNode('id').nodeValue === m[1] ? [
            p
          ] : B : [
          ]
        };
    ...
    (function () {
      ...
      if (g.getElementsByTagName('*').length > 0) o.find.TAG = function (i, n) {
        ...
        if (i[1] === '*') {
          for (var p = [
          ], q = 0; m[q]; q++) m[q].nodeType === 1 && p.push(m[q]);
          m = p
        }
        return m
      };
    ...
    filter: function (a, b, d) {
      if (d) a = ':not(' + a + ')';
      return b.length === 1 ? c.find.matchesSelector(b[0], a) ? [
        b[0]
      ] : [
      ] : c.find.matches(a, b)
    },
    ...
    sibling: function (a, b) {
      for (var d = [
      ]; a; a = a.nextSibling) a.nodeType === 1 && a !== b && d.push(a);
      return d
    }
  ...
  }; P.optgroup = P.option; P.tbody = P.tfoot = P.colgroup = P.caption = P.thead; P.th = P.td; if (!c.support.htmlSerialize) P._default = [
    1,
    'div<div>',
    '</div>'
  ]; c.fn.extend({
    ...
    remove: function (a, b) {
      for (var d = 0, e; (e = this[d]) != null; d++) if (!a || c.filter(a, [
        e
      ]).length) {
        if (!b && e.nodeType === 1) {
    ...
    html: function (a) {
      if (a === B) return this[0] && this[0].nodeType === 1 ? this[0].innerHTML.replace(za, '')  : null;
       else if (typeof a === 'string' && !Ca.test(a) && (c.support.leadingWhitespace || !$.test(a)) && !P[(Ba.exec(a) || ['',
      '']) [1].toLowerCase()]) {
        a = a.replace(Aa, '<$1></$2>');
        try {
    ...
    clean: function (a, b, d, e) {
      ...
      if (d) for (h = 0; f[h]; h++) if (e && c.nodeName(f[h], 'script') && (!f[h].type || f[h].type.toLowerCase() === 'text/javascript')) e.push(f[h].parentNode ? f[h].parentNode.removeChild(f[h])  : f[h]);
        ...
        f[h].nodeType === 1 && f.splice.apply(f, [
          h + 1,
          0
        ].concat(c.makeArray(f[h].getElementsByTagName('script'))));
        d.appendChild(f[h])
      }
      return f
    },
  ...
  'width'], function (a, b) {
    ...
    return f === '' ? 'auto' : f
  }; W = Ga || aa; if (c.expr && c.expr.filters) {
  ...
  var mb = c.now(), nb = /<script\b[^<]*(?:(?!<\/script>)<[^<]*)*<\/script>/gi, ob = /^(?:select|textarea)/i, pb = /^(?:color|date|datetime|email|hidden|month|number|password|range|search|tel|text|time|url|week)$/i, qb = /^(?:GET|HEAD)$/, Ra = /\[\]$/, T = /\=\?(&|$)/, ja = /\?/, rb = /([?&])_=[^&]*/, sb = /^(\w+:)?\/\/([^\/?#]+)/, tb = /%20/g, ub = /#.*$/, Ha = c.fn.load; c.fn.extend({
    load: function (a, b, d) {
    ...
    handleError: function (a, b, d, e) {
      ...
      a.global && c.triggerGlobal(a, 'ajaxError', [
        b,
        a,
        e
      ])
    ...
    handleComplete: function (a, b, d) {
      ...
      a.global && c.triggerGlobal(a, 'ajaxComplete', [
        b,
        a
      ]);
  ...
  }, vb = /^(?:toggle|show|hide)$/, wb = /^([+\-]=)?([\d+.\-]+)(.*)$/, ba, pa = [
    ['height',
    'marginTop',
    'marginBottom',
    'paddingTop',
 ...
 function PageZipper() {
  ...
  this.nextSynonyms = [
    ...
    },
    {
      ...
      humanReadableOnly: true,
      pageBar: true
    } //for nav bars that list pages 1234, list the next page
...
PageZipper.prototype.addMenu = function () {
  var css = '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t#pgzp_menu a, #pgzp_menu a * {border: 0; text-decoration: none;}\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t#pgzp_menu {position: fixed; top: 0px; float:left; padding: 0px 5px; background-color: #D3D3D3; color: black; z-index: 2147483647;}\t\t\t\t.pgzp_block {display: block; float: left; line-height: 32px;}\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t.pgzp_button {display: block; width: 32px; height: 32px;}\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\ta.pgzp_button_prev_active {background: transparent url(\'${media_path}32-gnome-prev.png\') no-repeat scroll top left; }\t\t\t\t\t\ta:hover.pgzp_button_prev_active {background-image: url(\'${media_path}32-gnome-prev_red.png\'); }\t\t\t\t\t\t\t\t\t\t\t\ta.pgzp_button_prev_inactive {background: transparent url(\'${media_path}32-gnome-prev_gray.png\') no-repeat scroll top left; }\t\t\t\ta.pgzp_button_next_active {background: transparent url(\'${media_path}32-gnome-next.png\') no-repeat scroll top left; }\t\t\t\t\t\ta:hover.pgzp_button_next_active {background-image: url(\'${media_path}32-gnome-next_red.png\'); }\t\t\t\t\t\t\t\t\t\t\t\ta.pgzp_button_next_inactive {background: transparent url(\'${media_path}32-gnome-next_gray.png\') no-repeat scroll top left; }\t\t\t\t#pgzp_button_compat {padding-left: 6px;}\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t.pgzp_button_compat_active {background: transparent url(\'${media_path}compat-mode.png\') no-repeat scroll 4px 4px;}\t\t\t\t\t\t.pgzp_button_compat_inactive {background: transparent url(\'${media_path}compat-mode-inactive.png\') no-repeat scroll 4px 4px;}\t\t\t\t\t#pgzp_curr_page {font-size: 24px;}\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t#pgzp_loaded_pages {font-size: 18px;}\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t'
  ;
  var html = '\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<div id=\'pgzp_menu\'>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t<a href=\'javascript:pgzp().goToNext(-1)\' id=\'pgzp_button_prev\' class=\'pgzp_block pgzp_button pgzp_button_prev_active\' title=\'Previous - Cntrl Up or Cntrl <\'></a>\t\t\t\t<a href=\'javascript:pgzp().goToNext(1)\' id=\'pgzp_button_next\' class=\'pgzp_block pgzp_button pgzp_button_next_active\' title=\'Next - Cntrl Down or Cntrl >\'></a>\t\t\t\t<div class=\'pgzp_block\' style=\'padding-left: 5px;\'><span id=\'pgzp_curr_page\' title=\'Current Page\'>1</span><span id=\'pgzp_loaded_pages\' title=\'Pages Loaded\'>/1</span></div>\t\t\t\t\t\t\t\t\t\t\t<a href=\'javascript:pgzp().toggleCompatMode()\' id=\'pgzp_button_compat\' class=\'pgzp_block pgzp_button pgzp_button_compat_inactive\' title=\'Compatibility Mode - Slower, but less likely to encounter problems\'></a>\t\t\t\t<a href=\'http://www.printwhatyoulike.com/pagezipper\' target=\'_blank\' title=\'PageZipper Home\' class=\'pgzp_block pgzp_button\' style=\'margin-left: -6px\'>\t\t\t\t\t\t\t<img src=\'${media_path}zipper_32.png\' alt=\'PageZipper!\' style=\'border-width: 0px\' />\t\t\t\t\t\t\t\t\t\t\t\t\t</a>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t</div>\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t\t'
  ;
...
PageZipper.prototype.trials = {
      ...
      for (i = 0; i < pgzp().nextSynonyms.length; i++) {
        ...
        if (nextLink.text.toLowerCase().indexOf(currWord.syn) >= 0) {
          if (currWord['humanReadableOnly']) {
            if (!nextLink.isHumanReadableText || //don't allow index as syn on urls
  ...
  'url_similarity': {
    'doScore': function (nextLink) {
      ...
      if (notMatchingPos > 0 &&
      pgzp().isNumber(longer.charAt(notMatchingPos)) && pgzp().isNumber(shorter.charAt(notMatchingPos)) && (Math.abs(pgzp().getNumberAtPos(shorter, notMatchingPos) - pgzp().getNumberAtPos(longer, notMatchingPos)) == 1)
      ) {
        score += 100;
      }
      return score;
...
PageZipper.prototype.getAutorunNextLink = function (body) {
  ...
  for (i = 1; i < scoredLinks.length; i++) {
    ...
    if ((scoredLinks[i][0] / scoredLinks[0][0]) > 0.7) {
      return null;
    } else {
...
PageZipper.prototype.getAllScoredLinks = function (body) {
  ...
  }); //filter out already loaded links, needed by pageBar, but not anymore
...
PageZipper.prototype.normalizeTrialSet = function (trialName, allLinks) {
  ...
  var highest,
  lowest = 0; //add zero so we don't normalize to 0 and 100 for urls which are 1 char apart
...
PageZipper.prototype.getCurrentPageNumberFromPageBar = function (allNextLinks) {
  ...
  if (pageBar[0].pageNum == 2) return [1,
  40];
  // if (pageBar[0].pageNum == 1) return [0, 10];
  //curr page is last page
  return [pageBar[pageBar.length - 1].pageNum,
  30];
}
//score page bar
...
PageZipper.prototype._runOnIframeLoad2 = function () {
  var ifrDoc = pgzp()._ril_ifr.contentWindow.document,
  validReadyStates = /loaded|interactive|complete/;
  ...
  if (ifrDoc.body && //checking for body b/c chrome has a bizarre habit of starting in readyState=complete and then switch to readyState=loading
...
PageZipper.prototype.addIframePlaceholder = function () {
  var placeholder = pgzp().jq('<div/>').attr('id', 'pgzp_iframe_placeholder').css({
    position: 'absolute',
    top: pgzp().jq(document).height() + 'px',
    left: '0px'
  });
