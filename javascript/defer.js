...
This shows some logic to keep track of callbacks bound to a given URL in order for the cache system to properly 
handle both complete and inbound requests.

One promise is cached per URL. If there is no promise for the given URL yet, then a deferred is created and the request is issued. 
If it already exists, however, the callback is attached to the existing deferred. The big advantage of this solution is that 
it will handle both complete and inbound requests transparently. 

Another advantage is that a deferred-based cache will deal with failure gracefully. The promise will end up rejected 
which can be tested for by providing an error callback:
...
$.cachedGetScript("http://127.0.0.1:8080/lib/fetch.js", function(t){console.dir(this.contents)});
$.cachedGetScript("http://127.0.0.1:8080/lib/filepicker.js", function(t){console.dir(this.contents)});
...
link Caching Data API responses
...
link Requesting panel content on first click and opening said panel
...
$('#myButton').firstClick(function () {
  var panel = $('#myPanel');
  $.when($.get('panel.html'), panel.slideDownPromise()
  ).done(function (ajaxResponse) {
    panel.html(ajaxResponse[0]).fadeIn();
  });
});

link Loading images in a panel on first click and opening said panel
...
link Loading images on the page after a specific delay
