...
function PageZipper() {
	...
	this.nextSynonyms = [
							...
							{syn: "antigua", weight: 50},
							{syn: "reciente", weight: 50},
...
//		if (!str) return;
//		var scripts = pgzp().doc.getElementsByTagName("script");
//		//get the last one
//		if (scripts.length > 0) {
...
//		}
	...
	pgzp().currDomain = pgzp().win.location.hostname;
...
//	pgzp().log("is loading: " + !pgzp().is_loading_page + " nextLink: " + pgzp().pages[pgzp().pages.length-1]["nextLink"] );
...
/*------------------------- Image Stuff ----------------------*/
//return array of all poster images on page url("http://www.printwhatyoulike.com/static/pagezipper/ui/32-gnome-prev_gray.png")
...
PageZipper.prototype.addMenu = function() {
	...
	var html = "																																							\
		<div id='pgzp_menu'>																																				\
			...
			<a href='http://www.printwhatyoulike.com/pagezipper' target='_blank' title='PageZipper Home' class='pgzp_block pgzp_button' style='padding-left: 5px'>												\
...
PageZipper.prototype.getNextLink = function(body) {
	...
	if (pgzp().pages.length > 1 && //don't check on first page
		!pgzp().pages[0].nextLink.isSynNumber() && !highestLink.isSynNumber() &&  //only compare if we are not using page indexes
		pgzp().pages[0].nextLink.syn != highestLink.syn)
		return null;
	return highestLink;
}
...
PageZipper.prototype.getCurrentPageNumberFromPageBar = function(allNextLinks) {
	...
	if (pageBar[0].pageNum == 2) return [1, 30];
	if (pageBar[0].pageNum == 1) return [0, 30];
...
PageZipper.prototype.captureEvent = function(event) {
	if (!event) event = pgzp().win.event;  //for IE
